 ...
 *   DaliTable       table_name     scope=schema_geoid  score=1.0
 *   DaliColumn      column_name    scope=table_geoid   score=0.9
 *   DaliOutputColumn name          scope=session_id    score=0.9
 *   DaliPackage     package_name   scope=schema_geoid  score=0.8
 *   DaliRoutine     routine_name   scope=schema_geoid  score=0.8
 *   DaliParameter   param_name     scope=session_id    score=0.75
 *   DaliVariable    var_name       scope=session_id    score=0.75
 *   DaliStatement   stmt_geoid     scope=session_id    score=0.6
 *   DaliSchema      schema_name    scope=db_name       score=0.7
 *   DaliDatabase    database_name  scope=db_name       score=0.7
...
public class SearchService {
    ...
    public Uni<List<SearchResult>> search(String query, int limit) {
        ...
        List<Uni<List<Map<String, Object>>>> unis = List.of(
            q("SELECT @rid AS rid, @type AS type, table_name     AS label, schema_geoid AS scope, 1.0  AS score FROM DaliTable        WHERE table_name     LIKE '%s' LIMIT %d", like, n),
            q("SELECT @rid AS rid, @type AS type, column_name    AS label, table_geoid  AS scope, 0.9  AS score FROM DaliColumn       WHERE column_name    LIKE '%s' LIMIT %d", like, n),
            ...
            q("SELECT @rid AS rid, @type AS type, package_name   AS label, schema_geoid AS scope, 0.8  AS score FROM DaliPackage      WHERE package_name   LIKE '%s' LIMIT %d", like, n),
            q("SELECT @rid AS rid, @type AS type, routine_name   AS label, schema_geoid AS scope, 0.8  AS score FROM DaliRoutine      WHERE routine_name   LIKE '%s' LIMIT %d", like, n),
            ...
            q("SELECT @rid AS rid, @type AS type, stmt_geoid     AS label, session_id   AS scope, 0.6  AS score FROM DaliStatement    WHERE stmt_geoid     LIKE '%s' LIMIT %d", like, n),
            ...
            q("SELECT @rid AS rid, @type AS type, database_name  AS label, db_name      AS scope, 0.7  AS score FROM DaliDatabase     WHERE database_name  LIKE '%s' LIMIT %d", like, n),
